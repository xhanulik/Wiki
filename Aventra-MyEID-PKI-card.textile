...
*NEW:* MyEID is certified by Microsoft and supports Smart Card Plug and Play.
...
In some environments there has been issues when reading files that exceed some threshold. If you encounter problems when reading larger files from the card (e.g. certificates) with no apparent reason, try to set the readers max_receive_size to e.g. 192, to be on the safe side. You can then try to iterate to find the maximum for your environment. 

The setting in the opensc.conf (usually in /etc) config file is the following:
...
<b>Card details:</b> "Reference manual":http://aventra.fi/pdf/MyEID%20PKI%20JavaCard%20Applet%20Reference%20Manual%201-7-7.pdf
...
"Aventra Ltd.":http://www.aventra.fi/English/index_E.php
"Web shop":https://services.aventra.fi/webshop/WebShop.aspx
...
!http://www.aventra.fi/Pictures/aventra_logo_white.PNG(Aventra logo)!
 ...
 * *Aventra's MyEID minidriver has passed Microsoft Windows Hardware Certification* and it will be available soon from Windows Update (Smart Card Plug and Play). You can also download it "here.":http://aventra.fi/English/downloads_Minidriver_E.html <small>(2013-01-11)</small>