h3. Update (2013-08-27)

DNIe support got merged by code from the OpenDNIe fork, by pull request #168. The code is now in Master. OpenSC software which you can download from this website still does <b>NOT</b> support the Spanish Ceres cards.


h3. Old (please change)


DNIe is one of the SpanishEid-s. There exists a patch for OpenSC which adds support for DNIe in OpenSC. "http://opendnie.cenatic.es/":http://opendnie.cenatic.es/


There are two different OpenSC implementations for Spanish National eID card (DNIe) support
* The official one provided by Spanish <i>Dirección General de la policía y de la Guardia Civil</i> (DGP) is based in OpenSC-0.11.8, released under GPLv3. It's not being currently mantained

* <b>OpenDNIe</b> is an alternate LGPL implementation, written from scratch based on several documents and forums around DNIe. It's intended to be integrated into OpenSC mainstream, so is actively mantained and synced with OpenSC repository

You can find the (unmaintained) original GPLv3 DGP's source code used at "Spanish Police web page":http://www.dnielectronico.es/descargas/index.html. OpenDNIe repository can be found at "Cenatic Foundation site":http://forja.cenatic.es/frs/?group_id=160

From the public administration point of view the card has been procured by the "Ministry of Interior (DGP's DNIe office)":http://www.dnielectronico.es/, The chip card is a <i>ST19WL34</i> provided by "ST Microelectrónics":http://www.st.com/internet/com/home/home.jsp and software has been procured by "Fabrica Nacional De Moneda y Timbre (FNMT-RCM)":http://www.cert.fnmt.es/index.php?lang=en.

The DNIe card software is closely related to ''FNMT's Ceres card'', being very similar in structure and design.

<b>OpenDNIe</b> is copyright 2011 of Juan Antonio Martinez < jonsito @ terra dot es >

Resources:
 * The official home page for the Spanish DNIe is "http://www.dnielectronico.es":http://www.dnielectronico.es
 * The OpenDNIe web page is "http://opendnie.cenatic.es":http://opendnie.cenatic.es
 * The main reference for DNIe internals is their Reference Manual: "http://forja.cenatic.es/docman/view.php/160/666/manual_tecnico_dnie.pdf":http://forja.cenatic.es/docman/view.php/160/666/manual_tecnico_dnie.pdf
 * Reference to card chip at ST site: "http://www.st.com/internet/mcu/product/85241.jsp":http://www.st.com/internet/mcu/product/85241.jsp
 * Link to Ceres Card info: "http://www.cert.fnmt.es/index.php?cha=cit&sec=9&page=85":http://www.cert.fnmt.es/index.php?cha=cit&sec=9&page=85
 * OpenSC Install & configure options for OpenDNIe card: "http://opendnie.cenatic.es/wiki/index.php/Documentacion_OpenDNIe_Instalacion":http://opendnie.cenatic.es/wiki/index.php/Documentacion_OpenDNIe_Instalacion
...
<pre><code>PKCS#15 Card [DNI electrónico]:
	Version        : 0
 ...
 * DNIe requieres PIN (CHV1) to access UserCertificates
...
h2. OpenDNIe implementation

Current implementation of DNIe OpenSC driver consists in:
 * "card-dnie.c":http://forja.cenatic.es/plugins/scmsvn/viewcvs.php/opensc-opendnie/trunk/src/libopensc/card-dnie.c?root=opendnie&view=log the (read-only) card driver
 * "pkcs15-dnie.c":http://forja.cenatic.es/plugins/scmsvn/viewcvs.php/opensc-opendnie/trunk/src/libopensc/pkcs15-dnie.c?root=opendnie&view=log a very simple emulation layer that fixes cert paths and ID's
 * "cwa14890.c":http://forja.cenatic.es/plugins/scmsvn/viewcvs.php/opensc-opendnie/trunk/src/libopensc/cwa-dnie.c?root=opendnie&view=log and "cwa14890.h":http://forja.cenatic.es/plugins/scmsvn/viewcvs.php/opensc-opendnie/trunk/src/libopensc/cwa-dnie.h?root=opendnie&view=log card-independent implementation for CWA14890 standard for Secure Messaging
 * "cwa-dnie.c":http://forja.cenatic.es/plugins/scmsvn/viewcvs.php/opensc-opendnie/trunk/src/libopensc/cwa-dnie.c?root=opendnie&view=log DNIe Data provider for cwa14890.c, that provides (local) keys and certificates required to complain with cwa14890 SM protocol
...
"OpenDNIe repository":http://forja.cenatic.es/plugins/scmsvn/viewcvs.php/?root=opendnie is synced with OpenSC mainstream trunk
There is also "nigthly builds":http://www.opensc-project.org/downloads/nightly/opendnie availables for Windows32/64 and MacOS
