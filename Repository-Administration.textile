h1. Repository Administration

[[PageOutline(2-3)]]

h2. Quick start #QuickStart

 * Manage repositories in the "Repository" admin panel, with `trac-admin` or in the `[repositories]` section of "Not found: text(trac.ini) link(TracIni#repositories-section)":wiki/TracIni#repositories-section.
...
h3. In `trac.ini` #ReposTracIni

Repositories and repository attributes can be specified in the `[repositories]` section of "Not found: text(trac.ini) link(TracIni#repositories-section)":wiki/TracIni#repositories-section. Every attribute consists of a key structured as `{name}.{attribute}` and the corresponding value separated with an equal sign (`=`). The name of the default repository is empty.

The main advantage of specifying repositories in `trac.ini` is that they can be inherited from a global configuration (see the "Not found: text(global configuration) link(TracIni#GlobalConfiguration)":wiki/TracIni#GlobalConfiguration section of TracIni). One drawback is that, due to limitations in the `ConfigParser` class used to parse `trac.ini`, the repository name is always all-lowercase.
...
h3. In the database #ReposDatabase
...
The following "trac-admin":wiki/TracAdmin commands can be used to perform repository operations from the command line.
...
h2. Repository synchronization #Synchronization
...
There is also new functionality in the form of a repository listener extension point <i>(IRepositoryChangeListener)</i> that is triggered by the post-commit hook when a changeset is added or modified, and can be used by plugins to perform actions on commit.

h3. Mercurial Repositories
...
h3. Explicit synchronization #ExplicitSync

This is the preferred method of repository synchronization. It requires setting the `[trac]  repository_sync_per_request` option in "Not found: text(trac.ini) link(TracIni#trac-section)":wiki/TracIni#trac-section to an empty value, and adding a call to `trac-admin` in the post-commit hook of each repository. Additionally, if a repository allows changing revision metadata, a call to `trac-admin` must be added to the post-revprop-change hook as well.
...
Note that you may have to set the environment variable PYTHON_EGG_CACHE to the same value as was used for the web server configuration before calling trac-admin, if you changed it from its default location. See "Trac Plugins":wiki/Trac-Plugins for more information.
...
<pre><code>#!application/x-dos-batch@C:\Python26\Scripts\trac-admin.exe C:\path\to\env changeset added "%1" "%2"
</code></pre>
...
<pre><code>#!application/x-dos-batch@C:\Python26\Scripts\trac-admin.exe C:\path\to\env changeset modified "%1" "%2"
</code></pre>
...
h3. Per-request synchronization #PerRequestSync

If the post-commit hooks are not available, the environment can be set up for per-request synchronization. In that case, the `[trac] repository_sync_per_request` option in "Not found: text(trac.ini) link(TracIni#trac-section)":wiki/TracIni#trac-section must be set to a comma-separated list of repository names to be synchronized.
...
h2. Migration from a single-repository setup (Subversion) #Migration
...
h2. Migration from a single-repository setup (Mercurial) #MigrationMercurial
...
h3. My trac-post-commit-hook doesn't work anymore #trac-post-commit-hook
