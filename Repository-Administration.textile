 ...
 * Manage repositories in the "Repository" admin panel, with `trac-admin` or in the `[repositories]` section of [[trac.ini|TracIni#repositories-section]].
...
Repositories and repository attributes can be specified in the `[repositories]` section of [[trac.ini|TracIni#repositories-section]]. Every attribute consists of a key structured as `{name}.{attribute}` and the corresponding value separated with an equal sign (`=`). The name of the default repository is empty.

The main advantage of specifying repositories in `trac.ini` is that they can be inherited from a global configuration (see the [[global configuration|TracIni#GlobalConfiguration]] section of TracIni). One drawback is that, due to limitations in the `ConfigParser` class used to parse `trac.ini`, the repository name is always all-lowercase.
...
The following [[trac-admin|TracAdmin]] commands can be used to perform repository operations from the command line.
...
This is the preferred method of repository synchronization. It requires setting the `[trac]  repository_sync_per_request` option in [[trac.ini|TracIni#trac-section]] to an empty value, and adding a call to `trac-admin` in the post-commit hook of each repository. Additionally, if a repository allows changing revision metadata, a call to `trac-admin` must be added to the post-revprop-change hook as well.
...
If the post-commit hooks are not available, the environment can be set up for per-request synchronization. In that case, the `[trac] repository_sync_per_request` option in [[trac.ini|TracIni#trac-section]] must be set to a comma-separated list of repository names to be synchronized.
