...
There is a "patch":https://github.com/hongquan/OpenSC-main/tree/gnuk being made to support "Gnuk":http://www.fsij.org/gnuk/, a variant of OpenPGP v.2. The patch is waiting for being merged to OpenSC upstream.

h2. Usage

To apply this usage to Gnuk, the patch above is needed. Basically, usage between general OpenPGP card (CryptoStick as example) and Gnuk are the same, except some differences will be noted below.
...
Deleting key is supported by Gnuk only. CryptoStick does not.
...
If you want to delete key from CryptoStick, the only option is to erase card (all things will be deleted).

h3. 5. Erase card (CryptoStick)

Erasing card is supported by CryptoStick (or general OpenPGP card v2) only. Gnuk does not support.
...
You can register and get certificate for free from "StartSSL":https://www.startssl.com/, using Firefox browser.
...
bc. pkcs15-init --delete-objects privkey,pubkey --id 3 --store-private-key mykey.pem --auth-id 3 --id 3
...
bc. pkcs15-init --delete-objects privkey,pubkey --id 3 --store-private-key myprivate.p12 --format pkcs12 --auth-id 3
...
bc. pkcs15-init --delete-objects privkey,pubkey --id 2 --store-private-key mykey.pem --auth-id 3 --id 2
