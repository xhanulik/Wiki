...
@pkcs15-nit@ also requires to explicitly remove existing key/object. That's why we have @--delete-objects privkey,pubkey --id 3@ in the command (though it has no effect to CryptoStick, which does not support deleting key, but support overwriting key).
