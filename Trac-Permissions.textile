...
| `BROWSER_VIEW` | View directory listings in the "repository browser":wiki/The-Trac-Repository-Browser |
| `LOG_VIEW` | View revision logs of files and directories in the "repository browser":wiki/The-Trac-Repository-Browser |
| `FILE_VIEW` | View files in the "repository browser":wiki/The-Trac-Repository-Browser |
| `CHANGESET_VIEW` | View "repository check-ins":wiki/Trac-Changeset-Module |

h3. Ticket System

| `TICKET_VIEW` | View existing "tickets":wiki/The-Trac-Ticket-System and perform [wiki:TracQuery ticket queries] |
| `TICKET_CREATE` | Create new "tickets":wiki/The-Trac-Ticket-System |
| `TICKET_APPEND` | Add comments or attachments to "tickets":wiki/The-Trac-Ticket-System |
...
| `TICKET_MODIFY` | Includes both `TICKET_APPEND` and `TICKET_CHGPROP`, and in addition allows resolving "tickets":wiki/The-Trac-Ticket-System. Tickets can be assigned to users through a [TracTickets#Assign-toasDrop-DownList drop-down list] when the list of possible owners has been restricted. |
...
| `ROADMAP_VIEW` | View the "roadmap":wiki/The-Trac-Roadmap page, is not (yet) the same as MILESTONE_VIEW, see [trac:#4292 #4292] |
| `ROADMAP_ADMIN` | to be removed with [trac:#3022 #3022], replaced by MILESTONE_ADMIN |

h3. Reports

| `REPORT_VIEW` | View "reports":wiki/Trac-Reports, i.e. the "view tickets" link. |
| `REPORT_SQL_VIEW` | View the underlying SQL query of a "report":wiki/Trac-Reports |
| `REPORT_CREATE` | Create new "reports":wiki/Trac-Reports |
| `REPORT_MODIFY` | Modify existing "reports":wiki/Trac-Reports |
| `REPORT_DELETE` | Delete "reports":wiki/Trac-Reports |
| `REPORT_ADMIN` | All `REPORT_*` permissions |

h3. Wiki System

| `WIKI_VIEW` | View existing "wiki":wiki/The-Trac-Wiki-Engine pages |
| `WIKI_CREATE` | Create new "wiki":wiki/The-Trac-Wiki-Engine pages |
| `WIKI_MODIFY` | Change "wiki":wiki/The-Trac-Wiki-Engine pages |
| `WIKI_RENAME` | Rename "wiki":wiki/The-Trac-Wiki-Engine pages |
| `WIKI_DELETE` | Delete "wiki":wiki/The-Trac-Wiki-Engine pages and attachments |
...
| `TIMELINE_VIEW` | View the "timeline":wiki/The-Trac-Timeline page |
| `SEARCH_VIEW` | View and execute "search":wiki/Using-Search queries |
...
You grant privileges to users using "trac-admin":wiki/TracAdmin. The current set of privileges can be listed with the following command:
