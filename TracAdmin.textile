...
Some of those operations can also be performed via the <i>Admin</i> web interface, an updated version of the [trac:WebAdmin] plugin now integrated within Trac (since version 0.11).
...
<pre><code>trac-admin /path/to/projenv wiki list
</code></pre>
...
<pre><code>trac-admin /path/to/projenv help
</code></pre>

Some commands have a more detailed help, which you can access by specifying the command's name as a subcommand for `help`:

<pre><code>trac-admin /path/to/projenv help <command>
</code></pre>

h3. `trac-admin <targetdir> initenv` #initenv
