 ...
 * Security Officer - SO
  ...
  Security officer is a person (or a role) who manages the smart card issuing process. A SO PIN can be asked for if modifications are done to the card (keys generated or certificates written), but the existence of a SO is not mandatory. 
 ...
 * The key is in an EF with ID h1. 0012 in the DF with ID = 3045, which on its turn is a DF with ID 5015, which on its turn is a DF of the MF (3F00).
...
Use the _pkcs15-tool --dump_ tool to see yourself what pkcs15 data is on
your card, or _opensc-explorer_ to browse through the files.
...
has not yet been implemented for all cards. The following cards support personalization:
[[ListTagged(card -readonly (supported or semisupported))]]
...
This is the main personalization tool that allows you
to do the all initialization things, e.g. add/delete keys, certificates, PINs and
data, generate keys, while specifying key usage, which PIN protects
which key etc, 

As said before, not all cards are supported for initialization. In that
...
Not all options might explained here (run "pkcs15-tool -h" to see them).
...
The --reader or -r can be given with any command. By default the first reader with a card
 ...
 * This operation may require a 'transport' key. pkcs15-init will ask you
   ...
   then try again, <b>DO NOT</b> try with the default key again or you may permanently lock your card!
 * If you want an SO PIN and PUK, you can specify them with the --so-pin and --so-puk options,
 ...
 * To specify the profile + option. The profile file can only be "pkcs15"
 ...
 * This operation may require a 'transport' key. pkcs15-init will ask you
   ...
   key will be used without asking. NOTE: if you get a "Failed to erase card:
   PIN code or key incorrect", the transport key is wrong. Find this key and
   then try again, <b>DO NOT</b> try the default key again or you may permanently lock your card!

h3. Add a PIN (not possible with the onepin profile option)
<pre><code>      pkcs15-init -P {-a <AuthID>} {--pin <PIN>} {--puk <PUK>} {-l <label>}
</code></pre>
...
<pre><code>      pkcs15-init -G <keyspec> -a <AuthID> --insecure {-i <ID>}
                  {-u <keyusage>}
...
<pre><code> pkcs15-init -G rsa/1024 -a 01 -l testkey
</code></pre>
 * Specify the AuthID of the PIN that protects this key (protect from being
   ...
   if you want the private key to be usable without first providing a PIN.
...
<pre><code>      pkcs15-init -S <keyfile> {-f <keyformat>} -a <AuthID> --insecure
...
<pre><code>      pkcs15-init -S <pkcs12file> -f PKCS12 -a <AuthID> {--insecure} {-i <ID>}
...
<pre><code>      pkcs15-init -X <certfile> {-f <certformat>} {-i <ID>} {--authority}
</code></pre>
...
h3. Add a public key
<pre><code>      pkcs15-init --store-public-key <keyfile> {-f <keyformat>} {-i <ID>}
                  {-l <label>}
</code></pre>
...
h3. Add data
<pre><code>      pkcs15-init -W <datafile> {-i <ID>} {-l <label>}
</code></pre>
...
<pre><code>      pkcs15-init -U <certfile> -f <format> -i <ID> {-a <pinid>}
</code></pre>
 ...
 * Specify the ID of the PIN needed to update the cert file (and if needed
   to delete it and create and write a new one) with -a.
 * Depending on your card and profile option, you will be prompted to provide
   your SO PIN and/or PIN; if you don't want to be prompted, add them to the
   command line with --so-pin <SOPIN> and/or --pin <PIN>.
 * <b>NOTE</b>: if the new cert is bigger then the old one, the tool will try to delete
...
<pre><code>      pkcs15-init -A <type> -i <ID> -l <label> {-a <pinid>}
</code></pre>
...
<pre><code>    engine dynamic -pre SO_PATH:engine_pkcs11 -pre ID:pkcs11 -pre LIST_ADD:1 -pre LOAD
</code></pre>
   to use the PKCS #11 engine
 * Then type (on the openssl command prompt)
<pre><code>    req -engine pkcs11 -new -key <ID> -keyform engine -out <cert_req>
</code></pre>
...
h2. Card-specific issues
 ...
 <i>"Experience is that marvelous thing that enables you to recognize a mistake when you make it again."</i> -- Franklin P. Jones
