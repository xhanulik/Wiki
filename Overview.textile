...
<b>Attention! Information on this page might be outdated or misleading. If in doubt, ask on [MailingLists opensc-devel mailing list]. Update to the wiki is work in progress. See "FrequentlyAskedQuestions":wiki/Frequently-Asked-Questions and "DeveloperInformation":wiki/Creating-applications-with-smart-card-support for now.</b>
...
"WindowsCSP":wiki/Windows-CSP-(CryptoAPI) for details. The advantage of Crypto API/CSP is that applications
...
"AppleCSP":wiki/Mac-OS-X-CSP for details. At this moment OpenSC does not support that interface
...
On Linux you might want to use the open source project "OpenCT":http://www.opensc-project.org/openct/
...
terminals that contain smart card readers. You can use "OpenCT":http://www.opensc-project.org/openct/
...
!attachments/wiki/OverView/OpenSC.png(OverView)!
