...
Example: Take a card with 2 PINs. Each PIN protects a private key and each private key has a corresponding cert chain. And then there are 3 other roots certs that have nothing to do with the other data. Now if num_slots h1. 4, hide_empty_tokens false; and if you put the card your second card reader, you'll get the following:
