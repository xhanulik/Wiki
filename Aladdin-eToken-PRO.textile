...
Aladdin provides their own software, which comprises both the middleware (necessary for all CardOS-based tokens) and the key-management tool ("also for Linux":http://www.etokenonlinux.org) which is not compatible with PKCS#15.  (However, as long as enough memory is available on the chip, it is possible to initialize the token with both OpenSC and this proprietary key manager, and thus install files and keys side by side - each software can then only handle their own structures.)
...
The middleware for Linux is available here:  ftp://ftp.ealaddin.com/pub/etoken/Linux ; and a third party provides the the "key-management tool for Linux":http://www.etokenonlinux.org (you don't need the latter if you just want your token to work with OpenSC).
