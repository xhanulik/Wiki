...
h3. 1. Update the Trac Code #UpdatetheTracCode
...
<pre><code># easy_install --upgrade Trac==0.12
</code></pre>
...
h3. 2. Upgrade the Trac Environment #UpgradetheTracEnvironment
...
<pre><code>trac-admin /path/to/projenv upgrade
</code></pre>
...
<pre><code>trac-admin /path/to/projenv upgrade --no-backup
</code></pre>

h3. 3. Update the Trac Documentation #UpdatetheTracDocumentation
...
<pre><code>trac-admin /path/to/projenv wiki upgrade
</code></pre>

Note that this procedure will leave your `WikiStart` page intact.

h3. 4. Resynchronize the Trac Environment Against the Source Code Repository
...
<pre><code>trac-admin /path/to/projenv repository resync '*'
</code></pre>

h3. 5. Refresh static resources
...
<pre><code>trac-admin /path/to/env deploy /deploy/path
</code></pre>
...
h3. 6. Steps specific to a given Trac version 


h4. Upgrading from Trac 0.11 to Trac 0.12


=h4. Python 2.3 no longer supported=
The minimum supported version of python is now 2.4

=h4. SQLite v3.x required=
...
=h4. Multiple Repository Support=
...
=h4. Improved repository synchronization=
...
h4. Upgrading from Trac 0.10 to Trac 0.11


=h4. Site Templates and Styles=
...
=h4. Web Admin plugin integrated=
If you had the webadmin plugin installed, you can uninstall it as it is part of the Trac code base since 0.11.

h3. 7. Restart the Web Server #RestarttheWebServer

If you are not running "CGI":wiki/Installing-Trac-as-CGI, reload the new Trac code by restarting your web server.
...
h2. Changing Database Backend


h3. SQLite to PostgreSQL
