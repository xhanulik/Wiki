...
For each revision log entry, there are 7 columns:
 1. The first column contains a pair of radio buttons and should be used 
    ...
    used for "Not found: text(viewing the actual changes) link(TracRevisionLog#viewingtheactualchanges)":wiki/TracRevisionLog#viewingtheactualchanges.
 1. A color code (similar to the one used for the
    "Not found: text(changesets) link(TracChangeset#ChangesetHeader)":wiki/TracChangeset#ChangesetHeader) indicating kind of change.
