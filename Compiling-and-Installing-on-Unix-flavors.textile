...
We suggest to install OpenSC into @/usr@ and to put the configuration file into @/etc/opensc@. The default
...
<pre><code>tar xfvz opensc-*.tar.gz
cd opensc-*
./bootstrap
 ...
 * "PCSC-Lite":https://pcsclite.alioth.debian.org/ (runtime and development)
...
<pre><code>su -c 'yum install readline-devel openssl-devel libxslt docbook-style-xsl pcsc-lite-devel automake autoconf libtool gcc'</code></pre>

If the certificates in the card are compressed, you also need zlib:

<pre><code>su -c 'yum install zlib-devel'
</code></pre>
...
<pre><code>export PKG_CONFIG_PATH=/usr/lib/pkgconfig:/usr/local/lib/pkgconfig:/opt/mystuff/liv/pkgconfig</code></pre>
