...
The SmartCard-HSM is available as USB stick, ID-1 card with contact/contactless interface as ID-000 plug-in and MicroSD card. It can be purchased at "cardomatic":http://www.cardomatic.biz/smartcards.php.
...
The resulting file contains a key description, the optional certificate and the key value encrypted under the DKEK. The key value and it's meta data is protected by a cryptogram checksum against modifications.
...
Only the first key custodian must enter the prime number and then the number of password shares defined by --pwd-shares-total are requested:
...
This is repeated until 3 key custodians entered their password share. The DKEK share password is then calculated from the provided shares, the DKEK share is decrypted and imported into the SmartCard-HSM.
