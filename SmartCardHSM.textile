...
The SmartCard-HSM is available as USB stick, ID-1 card with contact/contactless interface and as ID-000 plug-in. It can be purchased at "cardomatic":http://www.cardomatic.biz/smartcards.php.
...
A alternative and light-weight PKCS#11 module for read-only access is available with the "SmartCard-HSM for Embedded Devices":https://github.com/CardContact/sc-hsm-embedded/wiki/PKCS11 project.
...
The SmartCard-HSM does not permanently store public keys, but generates the required PKCS#11 public key object from certificates stored on the device. For newly generated key pairs without a certificate the certificate signing request is stored instead, however only RSA public key can be retrieved this way.
...
OpenSSL> req -engine pkcs11 -new -key id_10 -keyform engine -out cert.pem -text -x509 -days 3640
</code></pre>
...
* Operate multiple SmartCard-HSMs in cluster to improve performance and allow fail-over 
...
A DKEK is imported into a SmartCard-HSM using a preselected number of key shares. Each key share is given to a key custodian and only all key shares together assemble the DKEK. Key shares are individually imported and are assembled within SmartCard-HSM. Key shares can be imported independent of time and location, allowing to pass a half-initialized between key custodians until all share have been assembled.
...
When all key custodian have generated their key share, a SmartCard-HSM can be initialized and the DKEK can be created from these key shares (two in this example):
...
The sc-hsm-tool called without arguments reports that the import of DKEK shares is not completed as 2 shares are still missing.
...
As all keys share have been imported, the resulting DKEK is assembled internally and a key check value is calculated. The key check value represents the DKEK without disclosing it's value. Two SmartCard-HSMs with the same DKEK key check value contain the same DKEK. The DKEK key check value can be obtained at any time using the sc-hsm-tool command without arguments.
...
The resulting file contains a key description, the optional certificate and the key value encrypted under the DKEK. The file is protected by a cryptogram checksum against modifications.
...
The SmartCard-HSM now contains two identical keys, one with key ref 1 (the original key) and key ref 10 (the copy created during import).

The DKEK (and all stored keys) is erased in a subsequent initialization.

h3. Using a n-of-m threshold scheme
Control over a DKEK share can also be further split using a n-of-m threshold scheme for the DKEK share's password. In such a scheme m key custodians are given a share of the password and n key custodians must come together to decrypt the DKEK share for import.

The threshold scheme can be used to prevent a single point of failure. A lost DKEK share password will render key backups useless as the DKEK can't be re-established. If the DKEK share password is split amongst more key custodians than are necessary to reconstruct the share, then some may be lost without consequences.

Generating a DKEK share using the threshold scheme requires the arguments --pwd-shares-threshold and --pwd-shares-total. The later defines the number of password shares the former the number of shares required to calculate the password.
