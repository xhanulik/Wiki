...
These small rules for those who "<b>GetInvolved</b>":wiki/Getting-involved-in-OpenSC-development in OpenSC help the project produce consistent and predictable output, if followed. It is not a law set in stone but highly recommended.
 ...
 * Name things (including wiki pages) with the most appropriate name, preferably the name that appears in the official documentation of the card or product, without the extra ™ © ® symbols. This includes correct capitalization ("Siemens-CardOS-M4":wiki/Siemens-CardOS-M4 not cardOS or CardOs).
 ...
 * Git write access is granted to those who "GetInvolved":wiki/Getting-involved-in-OpenSC-development with OpenSC development and
  ...
  * Gerrit submit access is revoked after 12 months (1 year) of inactivity
 ...
 * Git rules (see also ReleaseHowto)
  ...
  * Do not mix the branch fed to "NightlyBuilds":wiki/Automated-OpenSC-builds machinery with topic branches. NightlyBuilds branches (except for master builds) are <b>supposed</b> to be rebased or deleted as one sees necessary.
  * Have topic branches.
 ...
 * Bigger patches should go through a review process on "opensc-devel":wiki/Mailing-lists (or as a Trac ticket, which is propagated to opensc-commits) for at least 3 days (72 hours) to give people time to review it. Developers have real lives as well.
