h1. Trac with FastCGI

"FastCGI":http://www.fastcgi.com/ interface allows Trac to remain resident much like with "mod_python":wiki/Trac-and-mod_python. It is faster than external CGI interfaces which must start a new process for each request. However, unlike mod_python, FastCGI supports "Apache SuEXEC":http://httpd.apache.org/docs/suexec.html, i.e. run with different permissions than web server. Additionally, it is supported by much wider variety of web servers.

<b>Note for Windows:</b> Trac's FastCGI does not run under Windows, as Windows does not implement `Socket.fromfd`, which is used by `_fcgi.py`. If you want to connect to IIS, you may want to try [trac:TracOnWindowsIisAjp AJP].

h2. Simple Apache configuration
...
<pre><code># Enable fastcgi for .fcgi files
...
<pre><code>FastCgiConfig -initial-env TRAC_ENV=/path/to/env/trac
</code></pre>
...
<pre><code>ScriptAlias /trac /path/to/www/trac/cgi-bin/trac.fcgi/
</code></pre>
...
<pre><code>DefaultInitEnv TRAC_ENV /path/to/env/trac/
</code></pre>

h4. alternative environment setup
...
<pre><code>ScriptAlias / /srv/tracsite/cgi-bin/trac.fcgi/
</code></pre>

h2. Simple Cherokee Configuration
...
h2. Simple Lighttpd Configuration
...
<pre><code>#var.fcgi_binary="/usr/bin/python /path/to/fcgi_frontend.py" # 0.11 if installed with easy_setup, it is inside the egg directory
...
<pre><code># Aliasing functionality is needed
...
<pre><code>#  This is for handling multiple projects
...
Other important information like "this updated TracInstall page":http://trac.lighttpd.net/trac/wiki/TracInstall, "Not found: text(and this) link(TracCgi#MappingStaticResources)":wiki/TracCgi#MappingStaticResources are useful for non-fastcgi specific installation aspects.
...
Note about running lighttpd with reduced permissions:
  ...
  If nothing else helps and trac.fcgi doesn't start with lighttpd settings `server.username h1. "www-data"`, `server.groupname "www-data"`, then in the `bin-environment` section set `PYTHON_EGG_CACHE` to the home directory of `www-data` or some other directory accessible to this account for writing.


h2. Simple LiteSpeed Configuration
...
<pre><code>DB Type: Password File
...
<pre><code>URI: /trac/                              <--- URI path to bind to python fcgi app we created	
...
<pre><code>#find/set base_rul, url, and link variables
...
<pre><code>trac@trac.example ~ $ ./trac-standalone-fcgi.py 
</code></pre>
...
----
See also:  TracGuide, TracInstall, "ModWSGI":wiki/Trac-and-mod_wsgi, [wiki:TracCgi CGI], [wiki:TracModPython ModPython], [trac:TracNginxRecipe TracNginxRecipe]
